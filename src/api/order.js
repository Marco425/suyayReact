import {BASE_PATH, ORDER_STATUS} from "../utils/constants"
...
export async function getOrdersByTableApi(idTable, status = "", ordering=""){
    try {
        ...
        const url = `${BASE_PATH}/api/orders/?${tableFilter}&${statusFilter}&${closeFilter}&${ordering}`;
...
export async function  checkDeliveredOrderApi(id){
    try {
        const url = `${BASE_PATH}/api/orders/${id}/`;
        const params = {
            method: "PATCH",
            headers: {
...
export async function  addPaymentToOrderApi(idOrder, idPayment){
    try {
        const url = `${BASE_PATH}/api/orders/${idOrder}/`;
        const params = {
            method: "PATCH",
            headers: {
                "Content-Type": "application/json"
...
export async function getOrdersByPaymentApi(idPayment){
    try {
        ...
        const url = `${BASE_PATH}/api/orders/?${paymentFilter}`;
        const response = await fetch(url);
        const result = await response.json();
        return result;
    } catch (error) {
